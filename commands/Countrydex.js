...
import { EphemeralReply, IntReply, IntReplyFile, EditReplyFile, DeferReply, EditReply } from "../utils/Interactions.js";
...
var ongoingTrades = new Map(), luckyMap = new Map(), listMap = new Map();

export { luckyMap, listMap };

export default {
    name: "countryballs",
    alias: [],
    category: "fun",
    description: "Utiliza Countrydex",
    args: true,
    usage: "<opción> <valor (opcional)> (Opciones disponibles: info, give, completion, about, favorite, luckyspin)",
    onlySlash: true,
    data: {
        type: 1,
        name: "countryballs",
        ...
        options: [{
            ...
            type: 1,
            name: "list",
            description: "Mira la lista de countryballs tuya o la de otros usuarios",
            options: [{
                type: 6,
                name: "member",
                description: "Miembro para ver los countryballs.",
                required: false
            }]
        }]
    },
    ...
    runSlash: async (ws, interaction) => {
        ...
        if (interaction.data.options[0].name == "last") {
            if (!owns.has(interaction.member.user.id) || owns.get(interaction.member.user.id)?.length == 0) return IntReply(interaction, { content: "¡Aún no has conseguido ningún countryball!" });
                ...
                if (Object.keys(cbObject).length == 0) return IntReply(interaction, { content: "No se encontró ese countryball.\nAsegúrate de usar la función de Autocompletar." })
                    ...
                    const image = await loadImage(path.resolve(`./assets/countrydex/cards/${cbObject.data.names[0]}.png`));
                    ctx.drawImage(image, 0, 0, 1428, 2000);
        ...
        if (interaction.data.options[0].name == "completion") {
            var gotNames = [], missingNames = [], renderedGotList = "", renderedMissingList = "**Countryballs faltantes**\n";
            ...
            countryballsInfo.countryballs.forEach((countryball, index) => {
                if (gotNames.includes(countryball.names[0])) return
                else missingNames.push(countryball.names[0])
            });

            if (missingNames.length == 0) renderedMissingList = "**__:tada: ¡Felicitaciones, has obtenido todos los countryballs! :tada:__**";
            else {
                var cache = [];
                countryballsInfo.countryballs.forEach((countryball, index) => {
                    if (missingNames.includes(countryball.names[0]) && !cache.includes(countryball.names[0])) {
                        if (index == 29 || ((index % 29) == 0 && index !== 0)) {
                            renderedMissingList += `\n\n${countryball.emoji} `
                        } else renderedMissingList += `${countryball.emoji} `
            ...
            countryballsInfo.countryballs.forEach((countryball, index) => {
                if (gotNames.includes(countryball.names[0])) {
                    if (index == 29 || ((index % 29) == 0 && index !== 0)) {
                        renderedGotList += `\n\n${countryball.emoji} `
                    } else renderedGotList += `${countryball.emoji} `
                }
                else return
            });
        ...
        if (interaction.data.options[0].name == "luckyspin") {
            ...
            IntReply(interaction, {
                embeds: [{
                    ...
                    image: { url: "https://cdn.discordapp.com/attachments/1091932806206201857/1099766690813849691/93_sin_titulo_20230423134005.png", height: 233, width: 289 },
                    footer: { text: `Solicitado por ${interaction.member.user.username}`, icon_url: ws.getAvatarURL(interaction.member.user) },
                }],
                components: [{
                    type: 1,
        ...
        if (interaction.data.options[0].name == "list") {
            ...
            const user = interaction.data.options[0].options.length == 1 ? interaction.data.options[0].options[0].value : interaction.member.user.id
            if (
                !owns.has(user) ||
                owns.get(user)?.length == 0
            ) return EditReply(interaction, { content: "¡El usuario aún no ha conseguido ningún countryball!" });

            var allOwns = owns.get(user);
            var ballArray = new Array();
            var optionsArray = new Array();

            allOwns.forEach(ball => {
                var catchDate = new Date(ball.catchDate * 1000)
                if (ball.shiny || ball.favorite) {
                    ballArray.unshift({
                        label: (ball.favorite ? "❤️ " : "") + (ball.shiny ? "✨ " : "") + ball.data.renderedName,
                        value: ball.id,
                        description: `HP: ${ball.percentages[0] > 0 ? "+" : ""}${ball.percentages[0]}% • ATK: ${ball.percentages[1] > 0 ? "+" : ""}${ball.percentages[1]}% • Atrapado el ${catchDate.getDate()}/${catchDate.getMonth() + 1}/${catchDate.getFullYear()} ${catchDate.getHours() < 10 ? "0" : ""}${catchDate.getHours()}:${catchDate.getMinutes() < 10 ? "0" : ""}${catchDate.getMinutes()}`,
                        emoji: {
                            id: ball.data.emoji.substring(5, ball.data.emoji.length - 1),
                            name: ball.data.emoji.substring(2, 4)
                        }
                    })
                } else {
                    ballArray.push({
            ...
            // Sort in groups of 25
            for (var i = 0; i < ballArray.length; i += 25) {

                var elements = ballArray.slice(i, i+25);
                optionsArray.push(elements)
             }

            const listId = generate(Date.now()).toString();
            var username = await GetMember(interaction.guild_id, user).then(r => r.user.global_name);
            listMap.set(listId, { page: 0, author: interaction.member.user.id, username: username, id: interaction.id, token: interaction.token, optionsArray, user });

            if (optionsArray.length == 1) {
                var finalResponse = {
                    content: "Lista de "+username,
                    components: [{
                        type: 1,
                        components: [{
                            type: 3,
                            custom_id: "countrydex_list",
                            options: optionsArray[0]
                        }]
                    }]
                }

                return EditReply(interaction, finalResponse);
            }

            var finalFullResponse = {
                content: "Lista de "+username,
                components: [{
                    type: 1,
                    components: [{
                        type: 3,
                        custom_id: "countrydex_list_"+user,
                        options: optionsArray[0]
                    }]
                }, {
                    type: 1,
            ...
            return EditReply(interaction, finalFullResponse);
        }
    }
}